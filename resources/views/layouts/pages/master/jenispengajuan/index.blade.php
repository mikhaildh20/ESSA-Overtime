        ...
        <div class="search-container">
            <form action="{{ route('jenis_pengajuan.index') }}" method="GET">
                <div class="input-group">
                    <!-- Create Button -->
                    <a href="{{ route('jenis_pengajuan.create') }}" class="btn btn-success">
        ...
        <table class="table table-bordered table-striped" id="jabatanTable">
            ...
            <tbody id="tableBody">
                ...
                <tr>
                    ...
                    <td>{{ $d->jpj_name }}</td>
                    <td>
                        <a href="{{ route('jenis_pengajuan.edit', $d->jpj_id) }}" class="btn btn-link btn-sm">
                        ...
                        @if($d->jpj_status == 1)
                            <button type="button" class="btn btn-link btn-sm" data-bs-toggle="modal" data-bs-target="#confirmModal" 
                                    data-action="{{ route('jenis_pengajuan.update_status', $d->jpj_id) }}" 
                                    data-status="inactive" data-id="{{ $d->jpj_id }}">
                                <i class="fas fa-trash-alt"></i>
                            </button>
                        @else
                            <button type="button" class="btn btn-link btn-sm" data-bs-toggle="modal" data-bs-target="#confirmModal" 
                                    data-action="{{ route('jenis_pengajuan.update_status', $d->jpj_id) }}" 
                                    data-status="active" data-id="{{ $d->jpj_id }}">
